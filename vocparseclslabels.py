...
class PascalVOC:
    ...
    def getImagePath(self, _xml):
        path = os.path.join(self.ann_dir, _xml + ".xml")
        mydoc = minidom.parse(path)
        return os.path.join(self.img_dir,
                            mydoc.getElementsByTagName('filename')[0]
...
if __name__=='__main__':
    ...
    # ip = pv.getImagePath(ls[0])
    # image = Image.open(ip)
    # image.show()

    print(len(ls),ls[0])
    print(pv.getImagePath(ls[0]))
